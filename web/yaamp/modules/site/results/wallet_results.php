...
// Secondary payout addresses & balances (merged mining / multi-coin payouts)
$sec_wallets = dbolist(
	"SELECT w.coinid, c.symbol, c.name, w.address, IFNULL(b.balance,0) AS balance "
	."FROM account_wallets w "
	."LEFT JOIN coins c ON c.id=w.coinid "
	."LEFT JOIN account_balances b ON b.account_id=w.account_id AND b.coinid=w.coinid "
	."WHERE w.account_id={$user->id} ORDER BY c.symbol"
);

if(count($sec_wallets))
{
	WriteBoxHeader("Secondary payouts (merged mining)");
	echo "<div style='padding:8px; font-size:13px;'>";
	echo "If you want to receive merged-mined coins (ex: DOGE) to a different wallet, set it in your miner password:\n";
	echo "<br><code>-p c={$refcoin->symbol},mc={$refcoin->symbol},da=YOUR_DOGE_ADDRESS</code> or <code>-p c={$refcoin->symbol},aw=DOGE:YOUR_DOGE_ADDRESS</code>";
	echo "</div>";

	echo "<table class='dataGrid2'>";
	echo "<thead><tr><th>Coin</th><th>Address</th><th align=right>Balance</th></tr></thead>";
	foreach($sec_wallets as $r) {
		$csym = htmlspecialchars($r['symbol']);
		$caddr = htmlspecialchars($r['address']);
		$cbal = altcoinvaluetoa($r['balance']);
		echo "<tr><td><b>{$csym}</b></td><td style='font-family:monospace'>{$caddr}</td><td align=right>{$cbal}</td></tr>";
	}
	echo "</table>";
	echo "</div>";
	echo "</div><br>";

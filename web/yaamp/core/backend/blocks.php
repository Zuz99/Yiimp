...
function BackendUpdatePoolBalances($coinid = NULL)
{
	...
	foreach($coins as $coin)
	{
		...
		$cleared_main = (double) dboscalar("SELECT SUM(balance) FROM accounts WHERE coinid=".intval($coin->id));
		$cleared_sec = (double) dboscalar("SELECT SUM(balance) FROM account_balances WHERE coinid=".intval($coin->id));
		$coin->cleared = $cleared_main + $cleared_sec;

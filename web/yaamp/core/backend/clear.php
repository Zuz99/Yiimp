...
// 		$value = $earning->amount * $coin->price / ($refcoin? $refcoin->price: 1);
		...
		// Multi-coin payouts when auto-exchange is OFF:
		// - if earning coin == user primary coin: credit accounts.balance (legacy)
		// - else: credit secondary balance (account_balances) in native coin units
		if(!YAAMP_ALLOW_EXCHANGE && intval($coin->id) != intval($user->coinid)) {
			yaamp_add_user_coin_balance($user->id, $coin->id, $earning->amount);
		} else {

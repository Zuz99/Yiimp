...
function BackendPayments()
{
	...
	// Coins can have balances either in accounts (primary coin) or in account_balances (secondary coins)
	$list = getdbolist('db_coins', "enable and (id in (select distinct coinid from accounts) OR id in (select distinct coinid from account_balances))");
...
//	debuglog("BackendCoinPayments $coin->symbol");
	...
	$users_main = getdbolist('db_accounts', "balance>$min_payout AND (payout_threshold IS NULL OR balance>payout_threshold) AND coinid={$coin->id} and !is_locked ORDER BY balance DESC");
	// secondary balances in native coin units
	$users_sec = dbolist(
		"SELECT b.account_id AS uid, b.balance AS balance "
		."FROM account_balances b INNER JOIN accounts a ON a.id=b.account_id "
		."WHERE b.coinid={$coin->id} AND b.balance>$min_payout AND (a.payout_threshold IS NULL OR b.balance>a.payout_threshold) AND !a.is_locked "
	...
	{
		foreach($users_main as $user)
		{
			$user = getdbo('db_accounts', $user->id);
			if(!$user) continue;
			$payaddr = yaamp_user_payout_address($user, $coin->id);
			if (empty($payaddr)) continue;

			$amount = $user->balance;
			while($user->balance > $min_payout && $amount > $min_payout)
			{
				debuglog("$coin->symbol sendtoaddress $payaddr $amount");
				$tx = $remote->sendtoaddress($payaddr, round($amount, 8));
				if(!$tx)
				{
					$error = $remote->error;
		...
		// Secondary balances (account_balances)
		foreach($users_sec as $row)
		{
			$uid = intval($row['uid']);
			...
			$payaddr = yaamp_user_payout_address($user, $coin->id);
			if (empty($payaddr)) {
				debuglog("{$coin->symbol} payout skipped: no secondary address set for user {$user->username}");
				continue;
			}
			...
			{
				debuglog("{$coin->symbol} sendtoaddress $payaddr $amount (secondary)");
				$tx = $remote->sendtoaddress($payaddr, round($amount, 8));
				if(!$tx)
				{
					$error = $remote->error;
				...
				// debit secondary balance
				dborun("UPDATE account_balances SET balance=balance-".floatval($amount).", updated=UNIX_TIMESTAMP() WHERE account_id=".$user->id." AND coinid=".$coin->id);
				$balance -= $amount;
	...
	foreach($users_main as $user)
	{
		$u = getdbo('db_accounts', $user->id);
		$addr = $u ? yaamp_user_payout_address($u, $coin->id) : null;
		if (empty($addr)) continue;
		$bal8 = round($user->balance, 8);
	...
	{
		$uid = intval($row['uid']);
		$u = getdbo('db_accounts', $uid);
		if (!$u) continue;
		$addr = yaamp_user_payout_address($u, $coin->id);
		if (empty($addr)) continue;
		$bal = round(floatval($row['balance']), 8);
		if ($bal <= 0) continue;
		$total_to_pay += $bal;
		$addresses[$addr] = ($addresses[$addr] ?? 0) + $bal;
	}
	...
	foreach($users_main as $user)
	{
		...
		if(!$user) continue;
		$payaddr = yaamp_user_payout_address($user, $coin->id);
		if(empty($payaddr) || !isset($addresses[$payaddr])) continue;

		$payment_amount = bitcoinvaluetoa($addresses[$payaddr]);
	...
	// create payout rows for secondary balances (account_balances)
	foreach($users_sec as $row)
	{
		$uid = intval($row['uid']);
		$user = getdbo('db_accounts', $uid);
		...
		if ($payout->save()) {
			$payouts[$payout->id] = $user->id;
			dborun("UPDATE account_balances SET balance=balance-".floatval($payment_amount).", updated=UNIX_TIMESTAMP() WHERE account_id=$uid AND coinid={$coin->id}");
		}
	}
	...
	$mailmsg = ''; $mailwarn = '';
	$failed_users = dbolist("SELECT DISTINCT account_id AS uid FROM payouts WHERE idcoin={$coin->id} AND IFNULL(tx,'') = ''");
	foreach($failed_users as $fu)
	{
		...
		$payaddr = yaamp_user_payout_address($user, $coin->id);
		if(empty($payaddr)) {
			$notice = "payment: Found payout(s) without tx but no payout address set for coin {$coin->symbol}, user {$user->username}!";
			debuglog($notice);
			$mailwarn .= "$notice\r\n";
			continue;
		}
		$amount_failed = 0.0;
		$failed = getdbolist('db_payouts', "account_id=:uid AND idcoin=:cid AND IFNULL(tx,'') = '' ORDER BY time", array(':uid'=>$user->id, ':cid'=>$coin->id));
		if (!empty($failed)) {
			if ($coin->symbol == 'CHC') {
				// tx made but payment rpc timed out
				foreach ($failed as $payout) $amount_failed += floatval($payout->amount);
				$notice = "payment: Found buggy payout without tx for {$payaddr}!! $amount_failed {$coin->symbol}";
				debuglog($notice);
				$mailwarn .= "$notice\r\n";
				continue;
			}
			...
			if ($amount_failed > 0.0) {
				debuglog("Found failed payment(s) for $payaddr, $amount_failed {$coin->symbol}!");
				if ($coin->rpcencoding == 'DCR') {
					$data = $remote->validateaddress($payaddr);
					if (!$data['isvalid']) {
						debuglog("Found bad address {$payaddr}!! ($amount_failed {$coin->symbol})");
						$user->is_locked = 1;
						$user->save();
						continue;
					}
				...
				if ($payout->save() && $amount_failed > $min_payout) {
					$payouts[$payout->id] = $user->id;
					$addresses[$payaddr] = $amount_failed;
					$mailmsg .= "{$amount_failed} {$coin->symbol} to {$payaddr} - user id {$user->id}\n";
				}
			}
		}
	}

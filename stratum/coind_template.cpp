...
YAAMP_JOB_TEMPLATE *coind_create_template(YAAMP_COIND *coind)
{
	...
	if (is_firopow) {
		...
		// Default: call getblocktemplate with no params unless a specific rule is needed.
		strcpy(params, "[]");
	}

	// Request GBT rules per-coin (DB flags).
	// Rules matrix:
	//  - usemweb=1   => ["segwit","mweb"]
	//  - usesegwit=1 => ["segwit"]
	//  - else        => [] (default params)
	if(!strcmp(coind->symbol, "PPC")) {
		strcpy(params, "[]");
	} else {
		const bool want_mweb = coind->usemweb;
		const bool want_segwit = coind->usesegwit || want_mweb;
		if(want_mweb) {

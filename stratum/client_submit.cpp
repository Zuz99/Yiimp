...
// Exact (256-bit) PoW target checks.
//
// Yiimp historically uses a truncated 64-bit compare (get_hash_difficulty/decode_compact)
// to decide if a share is a full block. That shortcut can create false "block found"
// detections (daemon rejects with "high hash").
//
// For share difficulty accounting we still keep the fast 64-bit path, but for deciding
// whether to submit a block (parent or aux) we use the proper 256-bit target compare.

static inline bool hash_meets_target_from_nbits(const unsigned char* hash_le, const char* nbits_hex)
{
	if(!hash_le || !nbits_hex || !nbits_hex[0]) return false;
	// nBits is a compact uint32 in hex (e.g. "1d00ffff").
	uint32_t nBits = (uint32_t) strtoul(nbits_hex, NULL, 16);
	bool fNegative = false;
	bool fOverflow = false;
	arith_uint256 target;
	target.SetCompact(nBits, &fNegative, &fOverflow);
	if(fNegative || fOverflow) return false;
	uint256 target256 = ArithToUint256(target);
	uint256 hash256;
	memcpy(hash256.begin(), hash_le, 32);
	return (hash256 <= target256);
}

static inline bool hash_meets_target_from_hex(const unsigned char* hash_le, const char* target_hex)
{
	if(!hash_le || !target_hex || !target_hex[0]) return false;
	uint256 target256;
	target256.SetHex(target_hex);
	uint256 hash256;
	memcpy(hash256.begin(), hash_le, 32);
	return (hash256 <= target256);
}
	...
	char *extranonce2, char *ntime, char *nonce, char *vote, uint32_t versionmask)
{
	...
	for(int i=0; i<templ->auxs_size; i++)
	{
		...
		const bool aux_block = hash_meets_target_from_hex(submitvalues->hash_bin, current_aux->target);
		if (aux_block && (!current_aux->skip_submitblock))
		{
	...
	const bool parent_block = hash_meets_target_from_nbits(submitvalues->hash_bin, templ->nbits);
	if(parent_block)
	{
		char count_hex[8] = { 0 };

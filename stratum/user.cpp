...
// Optional: per-user auxiliary coin payout addresses for merged-mining / dual wallets.
// Password format: aw=SYMBOL=ADDRESS/SYMBOL2=ADDRESS2/... (up to a handful of coins)
// Data is stored in account_wallets (see sql/2026-01-01-add_dual_wallets.sql).
static void db_set_account_wallets(YAAMP_DB *db, int account_id, const std::string& aw)
{
	if(!db || account_id <= 0 || aw.empty()) return;

	std::vector<std::string> pairs;
	string_tokenize(aw, '/', pairs);
	...
	for (auto &pair : pairs) {
		...
		std::string sym = pair.substr(0, eq);
		std::string addr = pair.substr(eq+1);
		if(sym.empty() || addr.empty()) continue;
		// basic injection hardening
		char symbuf[32] = {0};
		char addrbuf[256] = {0};
		strncpy(symbuf, sym.c_str(), sizeof(symbuf)-1);
		strncpy(addrbuf, addr.c_str(), sizeof(addrbuf)-1);
		db_check_user_input(symbuf);
		db_check_user_input(addrbuf);
		if(strlen(symbuf) < 2 || strlen(symbuf) > 12) continue;
		if(strlen(addrbuf) < MIN_ADDRESS_LEN) continue;

		// find coin id
		db_query(db, "SELECT id FROM coins WHERE installed AND (symbol='%s' OR symbol2='%s') LIMIT 1", symbuf, symbuf);
		MYSQL_RES *result = mysql_store_result(&db->mysql);
		if(!result) continue;
		MYSQL_ROW row = mysql_fetch_row(result);
		int coinid = row && row[0] ? atoi(row[0]) : 0;
		mysql_free_result(result);
		...
		db_query(db,
			...
			account_id, coinid, addrbuf, now, now);

		if(++count >= 8) break; // safety cap (supports 4-6+ coins)
	}
}
...
void db_add_user(YAAMP_DB *db, YAAMP_CLIENT *client)
{
	...
	for (auto &command_pairs: commandlist) {
		// special-case: aw=SYMBOL=ADDR/SYMBOL2=ADDR2...
		if(command_pairs.rfind("aw=", 0) == 0) {
			auxwallets = command_pairs.substr(3);
			continue;
		}
	...
	// Store optional aux/dual-wallet addresses (aw=...)
	if(client->userid > 0 && !auxwallets.empty()) {
		db_set_account_wallets(db, client->userid, auxwallets);
	}
}
